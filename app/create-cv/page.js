...
import { CVComponents, colorOptions, DEFAULT_DESIGN } from '../components/CVTemplates'
...
const SECTION_LABELS={summary:'Özet/Hakkımda',experience:'Deneyim',education:'Eğitim',skills:'Beceriler'}
...
const Ic={
  ...
  drag: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16"/></svg>,
}
...
function Slider({label,value,onChange,min=0,max=200}){
  return(
    <div>
      <div className="flex justify-between mb-1"><span className="text-gray-400 text-xs">{label}</span><span className="text-gray-500 text-xs tabular-nums">{value}%</span></div>
...
/* Seçim Butonu — uniform */
function OptionBtn({active,onClick,children,className=''}){
  return(
    <button onClick={onClick}
      className={`px-3 py-2 rounded-xl text-xs font-medium border transition-all ${active?'bg-blue-600 border-blue-500 text-white':'bg-gray-800 border-gray-700 text-gray-400 hover:text-white hover:border-gray-600'} ${className}`}>
      {children}
    </button>
  )
}
...
function CreateCVContent(){
  const sp=useSearchParams(), isEdit=sp.get('edit')==='1'
  ...
  const [filters,setFilters]=useState({brightness:100,contrast:100,saturation:100,shape:'circle',preset:'normal'})
  ...
  // Tools
  const [scoreData,setScoreData]=useState(null),[scoring,setScoring]=useState(false)
  const [translating,setTranslating]=useState(false),[currentLang,setCurrentLang]=useState('tr')
  const [linkedinTxt,setLinkedinTxt]=useState(null),[genLinkedin,setGenLinkedin]=useState(false)
  const [refLetter,setRefLetter]=useState(null),[genRef,setGenRef]=useState(false)
  const [showRefForm,setShowRefForm]=useState(false)
  const [refInfo,setRefInfo]=useState({refName:'',refPosition:'',refCompany:'',relationship:''})
  const [editMode,setEditMode]=useState(false),[editData,setEditData]=useState(null)
  const [shareLink,setShareLink]=useState(null),[sharing,setSharing]=useState(false)
  const [showMobilePreview,setShowMobilePreview]=useState(false)

  // Drag state for section order
  ...
  const reorderSections=(fromIdx,toIdx)=>{
    const arr=[...design.sectionOrder]
    const [item]=arr.splice(fromIdx,1)
    arr.splice(toIdx,0,item)
    setD('sectionOrder',arr)
  }

  const prevCss=[
    `brightness(${filters.brightness}%)`,`contrast(${filters.contrast}%)`,`saturate(${filters.saturation}%)`,
    filters.preset==='bw'?'grayscale(100%)':'',filters.preset==='sepia'?'sepia(80%)':'',
    filters.preset==='warm'?'sepia(30%) saturate(140%)':'',filters.preset==='cool'?'hue-rotate(20deg) saturate(90%)':'',
  ].filter(Boolean).join(' ')
  ...
  const handlePhotoFile=e=>{
    ...
    r.onload=async ev=>{
      const src=ev.target.result;setRawPhoto(src);setFilters(f=>({...f,brightness:100,contrast:100,saturation:100,preset:'normal'}))
      const p=await bakePhoto(src,{brightness:100,contrast:100,saturation:100,shape:filters.shape,preset:'normal'});setCvPhoto(p)
  ...
  const removePhoto=()=>{
    setRawPhoto(null);setCvPhoto(null)
  ...
  const handleImprove=async improve=>{
    ...
    if(pendingPhoto64){setRawPhoto(pendingPhoto64);const p=await bakePhoto(pendingPhoto64,{...filters,brightness:100,contrast:100,saturation:100,preset:'normal'}).catch(()=>null);if(p)setCvPhoto(p)}
  ...
  const generateCV=async(improveContent=true)=>{
    ...
    try{
      ...
      const data=await res.json()
      if(data.success){
        const photo=cvPhoto||(pendingPhoto64?await bakePhoto(pendingPhoto64,{...filters,brightness:100,contrast:100,saturation:100,preset:'normal'}).catch(()=>null):undefined)
  ...
  const photoPanel=(
    <div className="space-y-4">
      <div className="flex justify-center py-1">
        <div className={`w-24 h-24 overflow-hidden bg-gray-800 border-2 border-gray-700 flex items-center justify-center ${SHAPES.find(s=>s.id===filters.shape)?.cls||'rounded-full'}`}>
          {photoLoading?<div className="w-7 h-7 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"/>
           :rawPhoto?<img src={rawPhoto} className="w-full h-full object-cover" style={{filter:prevCss}} alt=""/>
        ...
        <>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {SHAPES.map(s=><OptionBtn key={s.id} active={filters.shape===s.id} onClick={()=>setFilters(f=>({...f,shape:s.id}))}>{s.label}</OptionBtn>)}
            </div>
          </div>
          <div>
  ...
  const designPanel=(
    <div className="space-y-5">
      ...
      {/* Yazı Tipi */}
      <div>
        <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-2">Yazı Tipi</p>
        <div className="grid grid-cols-3 gap-1.5">
          {[
            {id:'sans', label:'Modern',  preview:'Aa',fontClass:'font-sans' },
            {id:'serif',label:'Klasik',  preview:'Aa',fontClass:'font-serif'},
            {id:'mono', label:'Teknik',  preview:'Aa',fontClass:'font-mono' },
          ].map(f=>(
            <button key={f.id} onClick={()=>setD('font',f.id)}
              className={`py-2.5 flex flex-col items-center gap-1 rounded-xl border transition-all ${design.font===f.id?'bg-blue-600/10 border-blue-500 text-blue-400':'bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-600'}`}>
              <span className={`text-lg font-bold leading-none ${f.fontClass} ${design.font===f.id?'text-blue-400':'text-gray-300'}`}>{f.preview}</span>
      ...
      {/* Yoğunluk */}
      <div>
        <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-2">Satır Yoğunluğu</p>
        <div className="grid grid-cols-3 gap-1.5">
          {[
            {id:'compact',label:'Kompakt',icon:'▪▪▪'},
            {id:'normal', label:'Normal', icon:'▪ ▪ ▪'},
            {id:'wide',   label:'Geniş',  icon:'▪  ▪  ▪'},
          ].map(d=>(
            <OptionBtn key={d.id} active={design.density===d.id} onClick={()=>setD('density',d.id)}>
              <div className="text-center">
                <div className="text-xs mb-0.5 tracking-widest">{d.icon}</div>
                <div>{d.label}</div>
              </div>
            </OptionBtn>
          ))}
      ...
      {/* Başlık Stili */}
      <div>
        <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-2">Başlık Stili</p>
        <div className="grid grid-cols-2 gap-1.5">
          {[
            {id:'uppercase',label:'BÜYÜK HARF',  preview:'DENEYIM'},
            {id:'underline',label:'Alt Çizgili', preview:'Deneyim'},
            {id:'colored',  label:'Renkli',      preview:'Deneyim'},
            {id:'dotted',   label:'Çizgili',     preview:'Deneyim'},
          ].map(h=>(
            <button key={h.id} onClick={()=>setD('headingStyle',h.id)}
              className={`py-2 px-3 rounded-xl border text-left transition-all ${design.headingStyle===h.id?'bg-blue-600/10 border-blue-500':'bg-gray-800 border-gray-700 hover:border-gray-600'}`}>
              <p className={`text-xs font-bold mb-0.5 ${design.headingStyle===h.id?'text-blue-400':'text-gray-300'} ${h.id==='uppercase'?'uppercase tracking-widest text-[10px]':''} ${h.id==='underline'?'border-b border-gray-400 pb-0.5 inline-block':''}`}>{h.preview}</p>
              <p className={`text-xs ${design.headingStyle===h.id?'text-blue-300':'text-gray-600'}`}>{h.label}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Vurgu Yoğunluğu */}
      <div>
        <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-2">Renk Yoğunluğu</p>
        <div className="grid grid-cols-3 gap-1.5">
          {[
            {id:'light', label:'Hafif', desc:'Minimal'},
            {id:'medium',label:'Orta',  desc:'Standart'},
            {id:'strong',label:'Güçlü', desc:'Belirgin'},
          ].map(i=>(
            <button key={i.id} onClick={()=>setD('intensity',i.id)}
              className={`py-2 px-2 rounded-xl border text-center transition-all ${design.intensity===i.id?'bg-blue-600/10 border-blue-500':'bg-gray-800 border-gray-700 hover:border-gray-600'}`}>
              <p className={`text-xs font-medium ${design.intensity===i.id?'text-blue-400':'text-gray-300'}`}>{i.label}</p>
              <p className={`text-[10px] ${design.intensity===i.id?'text-blue-300/70':'text-gray-600'}`}>{i.desc}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Ayırıcı Stili */}
      <div>
        <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-2">Öğe Ayırıcısı</p>
        <div className="grid grid-cols-3 gap-1.5">
          {[
            {id:'line',label:'Çizgi', icon:'──'},
            {id:'dot', label:'Nokta', icon:'• • •'},
            {id:'none',label:'Yok',   icon:'   '},
          ].map(s=>(
            <OptionBtn key={s.id} active={design.separator===s.id} onClick={()=>setD('separator',s.id)}>
              <div className="text-center">
                <div className="text-xs mb-0.5 opacity-70">{s.icon}</div>
                <div>{s.label}</div>
              </div>
            </OptionBtn>
          ))}
      ...
      {/* Bölüm Sırası — Drag & Drop */}
      <div>
        <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1">Bölüm Sırası</p>
        <p className="text-gray-600 text-xs mb-2">Sürükle & bırak ile sıralamayı değiştir</p>
        <div className="space-y-1.5">
          {design.sectionOrder.map((section,idx)=>(
            <div key={section}
              draggable
              ...
              className={`flex items-center gap-2.5 bg-gray-800 hover:bg-gray-750 border rounded-xl px-3 py-2.5 cursor-grab active:cursor-grabbing transition-all select-none ${dragIdx===idx?'opacity-40 border-blue-500':'border-gray-700'}`}>
              <span className="text-gray-600 shrink-0">{Ic.drag}</span>
              <span className="text-gray-300 text-xs font-medium flex-1">{SECTION_LABELS[section]||section}</span>
              <div className="flex gap-0.5">
                {idx>0&&<button onClick={()=>reorderSections(idx,idx-1)} className="w-5 h-5 flex items-center justify-center text-gray-600 hover:text-white text-xs">↑</button>}
                {idx<design.sectionOrder.length-1&&<button onClick={()=>reorderSections(idx,idx+1)} className="w-5 h-5 flex items-center justify-center text-gray-600 hover:text-white text-xs">↓</button>}
              </div>
            </div>
          ))}
        </div>
        <button onClick={()=>setD('sectionOrder',[...DEFAULT_DESIGN.sectionOrder])}
          className="w-full mt-2 text-gray-600 hover:text-gray-400 text-xs py-1.5 border border-gray-800 rounded-xl hover:border-gray-700 transition-all">
          ↺ Varsayılana Sıfırla
        </button>
      </div>
    ...
    <div className="space-y-2.5">
      {!cvData
        ...
        :<>
            ...
            {shareLink&&(<div className="bg-gray-800/60 border border-green-800/50 rounded-xl p-3"><p className="text-green-400 text-xs mb-2">✅ Link hazır!</p><div className="flex gap-1.5"><input value={shareLink} readOnly className="flex-1 bg-gray-800 text-white rounded-lg px-2.5 py-1.5 text-xs outline-none min-w-0"/><button onClick={()=>{navigator.clipboard.writeText(shareLink);alert('Kopyalandı!')}} className="bg-green-600 text-white text-xs px-3 py-1.5 rounded-lg shrink-0">Kopyala</button></div></div>)}
    ...
    <div className="space-y-3">
      {!cvData
        ...
        :<>
            ...
            {scoreData&&(<div className="bg-gray-800/40 border border-gray-700/40 rounded-xl p-3"><div className="flex justify-between items-center mb-2"><span className="text-white text-xs font-medium">Toplam Puan</span><span className={`text-xl font-bold ${(scoreData.totalScore??0)>=80?'text-green-400':(scoreData.totalScore??0)>=60?'text-yellow-400':'text-red-400'}`}>{scoreData.totalScore??scoreData.toplam_puan??'—'}<span className="text-xs text-gray-600 font-normal">/100</span></span></div><div className="w-full bg-gray-800 rounded-full h-1.5 mb-2"><div className="bg-blue-500 h-1.5 rounded-full" style={{width:`${scoreData.totalScore??scoreData.toplam_puan??0}%`}}/></div>{scoreData.strengths?.slice(0,2).map((s,i)=><p key={i} className="text-gray-500 text-xs">💪 {s}</p>)}{scoreData.improvements?.slice(0,2).map((s,i)=><p key={i} className="text-gray-500 text-xs">🔧 {s}</p>)}</div>)}
            <div className="grid grid-cols-2 gap-1.5">
              <button onClick={()=>translate('en')} disabled={translating||currentLang==='en'} className="flex items-center justify-center gap-1 bg-gray-800 hover:bg-gray-700 disabled:opacity-40 text-gray-300 hover:text-white text-xs py-2.5 rounded-xl border border-gray-700/50">{Ic.globe} 🇬🇧 İngilizce</button>
              <button onClick={()=>translate('tr')} disabled={translating||currentLang==='tr'} className="flex items-center justify-center gap-1 bg-gray-800 hover:bg-gray-700 disabled:opacity-40 text-gray-300 hover:text-white text-xs py-2.5 rounded-xl border border-gray-700/50">{Ic.globe} 🇹🇷 Türkçe</button>
            </div>
            ...
            <div className="space-y-3">
              <button onClick={()=>{setStartMode('upload');setTimeout(()=>fileRef.current?.click(),50)}}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0"><h3 className="text-white font-bold text-sm mb-0.5">Belge Yükle</h3><p className="text-gray-500 text-xs">Mevcut CV'nizi yükleyin, AI profesyonel hale getirir.</p></div>
                  <div className="flex gap-1 shrink-0">{['PDF','DOCX','TXT'].map(f=><span key={f} className="text-xs bg-gray-800 text-gray-500 px-1.5 py-0.5 rounded border border-gray-700/50">{f}</span>)}</div>
                </div>
              </button>
              <button onClick={()=>setStartMode('text')}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1"><h3 className="text-white font-bold text-sm mb-0.5">Manuel Yaz</h3><p className="text-gray-500 text-xs">Kısa notlar yazın, AI profesyonel CV'ye dönüştürsün.</p></div>
                  <span className="text-xs bg-purple-900/30 text-purple-400 px-2 py-0.5 rounded border border-purple-800/40 shrink-0">AI</span>
                </div>
              </button>
            </div>
            <div className="text-center mt-6"><button onClick={()=>router.push('/dashboard')} className="text-gray-600 hover:text-gray-400 text-sm">← Dashboard'a dön</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[70] bg-black/80 backdrop-blur-sm lg:hidden" onClick={()=>setShowMobilePreview(false)}>
          <div className="absolute inset-x-0 bottom-0 top-16 flex flex-col" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 bg-gray-900 border-b border-gray-800 shrink-0">
              <div><p className="text-white text-sm font-semibold">CV Önizleme</p><p className="text-gray-500 text-xs">{template} · {color}</p></div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={()=>setShowMobilePreview(false)} className="w-8 h-8 flex items-center justify-center bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white rounded-lg">{Ic.close}</button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto bg-gray-100 p-3">
              <CVComponent cvData={cvData||previewData} color={color} design={design}/>
            </div>
          </div>
        </div>
      )}
      ...
      {(startMode==='done'||isEdit)&&(
        <div className={`flex ${NAV_H_CLS} overflow-hidden`}>
          ...
          <div className="hidden lg:flex flex-col items-center gap-1 py-4 px-2 bg-[#0d1117] border-r border-gray-800/80 w-14 shrink-0">
            <button onClick={()=>router.push('/dashboard')} title="Geri" className="w-9 h-9 flex items-center justify-center rounded-xl text-gray-600 hover:text-white hover:bg-gray-800 mb-3">{Ic.back}</button>
          ...
          <div className="w-full lg:w-72 xl:w-80 shrink-0 bg-gray-950 border-r border-gray-800/60 flex flex-col overflow-hidden">
            ...
            <div className="lg:hidden shrink-0 bg-gray-900/80 border-b border-gray-800/60">
              <div className="flex items-center gap-2 px-4 pt-3 pb-2">
                <button onClick={()=>router.push('/dashboard')} className="flex items-center gap-1.5 text-gray-400 hover:text-white text-xs bg-gray-800 hover:bg-gray-700 px-2.5 py-1.5 rounded-lg border border-gray-700/50 shrink-0">{Ic.back}<span>Geri</span></button>
                <span className="text-white text-sm font-semibold">CV Oluştur</span>
                {cvData&&<span className="ml-auto flex items-center gap-1 text-green-400 text-xs"><span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"/>Hazır</span>}
              </div>
              <div className="flex border-t border-gray-800/60">
                {TABS.map(tab=>(
                  <button key={tab.id} onClick={()=>setActiveTab(tab.id)}
                    className={`relative flex-1 flex flex-col items-center gap-1 py-2.5 transition-all ${activeTab===tab.id?'text-blue-400 border-b-2 border-blue-500':'text-gray-600 hover:text-gray-400'}`}>
                ...
                <button onClick={()=>setShowMobilePreview(true)} className="flex-1 flex flex-col items-center gap-1 py-2.5 text-gray-600 hover:text-blue-400 transition-all">
                  {Ic.eye}<span className="text-[10px] leading-none font-medium text-gray-600">Önizle</span>
                </button>
              </div>
            </div>
            ...
            <div className="hidden lg:flex items-center shrink-0 px-4 py-3 border-b border-gray-800/40">
              ...
              {cvData&&activeTab!=='actions'&&<button onClick={()=>setActiveTab('actions')} className="ml-auto text-blue-400 text-xs hover:text-blue-300">Eylemler →</button>}
            </div>

            <div className="flex-1 overflow-y-auto px-4 py-4 pb-8">{panelMap[activeTab]}</div>
          </div>
