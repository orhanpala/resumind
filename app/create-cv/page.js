...
/* ── Canvas ile fotoğrafı akıllıca kırpar (yüz bölgesi üst-orta) ── */
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        /* Kare kırpma — yüzler genellikle üst-orta bölgede */
        const side = Math.min(img.width, img.height)
        const srcX = (img.width - side) / 2
        /* Dikeyde %10 yukarı kaydır (yüz bölgesi) */
        ...
        ctx.drawImage(img, srcX, srcY, side, side, 0, 0, OUTPUT, OUTPUT)
...
function CreateCVContent() {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      setCvPhoto(cropped)
      /* Mevcut cvData varsa fotoğrafı ona da ekle */
      if (cvData) {
        const updated = { ...cvData, photo: cropped }
        setCvData(updated)
        setPreviewData(updated)
      }
    } catch {
      alert('Fotoğraf işlenemedi, başka bir görsel deneyin.')
    }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.success) {
        const newCV = { ...data.cvData, photo: cvPhoto || undefined }
        setCvData(newCV)
        setPreviewData(newCV)
        const { data: { user } } = await supabase.auth.getUser()
        if (user) {
          await supabase.from('cvs').insert({ user_id: user.id, template, cv_data: newCV })
        }
  ...
  const handleDownloadPDF = () => {
    ...
    if (!element) return alert('CV önizlemesi bulunamadı.')
  ...
  const handleTranslate = async (lang) => {
    ...
    try {
      ...
      if (data.success) {
        const t = { ...data.translatedCV, photo: cvPhoto || undefined }
        setCvData(t); setPreviewData(t); setCurrentLang(lang)
  ...
  const handleReferenceLetter = async () => {
    ...
    try {
      const res = await fetch('/api/reference-letter', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const { data: { user } } = await supabase.auth.getUser()
      if (user) {
        const { data: ex } = await supabase.from('cvs').select('id').eq('user_id', user.id)
          .eq('template', template).order('created_at', { ascending: false }).limit(1).single()
  ...
  return (
    <div className="min-h-screen bg-gray-950 pt-16">
      <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)]">
        ...
        {/* ══ SOL PANEL ══ */}
        <div className="w-full lg:w-[480px] lg:max-w-[480px] xl:w-[520px] bg-gray-950 lg:overflow-y-auto lg:h-[calc(100vh-64px)] lg:sticky lg:top-16 flex-shrink-0">

          {/* ── Sticky sub-header (Geri butonu) ── */}
          ...
          <div className="px-4 sm:px-6 py-5 space-y-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <label className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-blue-500 text-white text-xs px-4 py-2.5 rounded-xl cursor-pointer transition-all w-full justify-center">
                    {cvPhoto ? '📷 Fotoğrafı Değiştir' : '📷 Fotoğraf Yükle'}
            ...
            {mode && !cvData && (
              <div>
                ...
                {loading
                  ? <div className="bg-gray-900 border border-gray-800 rounded-2xl"><LoadingAnimation text="CV oluşturuluyor, lütfen bekleyin..." /></div>
                  : <button onClick={handleGenerate} disabled={!cvText}
            ...
            {cvData && (
              <div className="space-y-2 pb-10">
                ...
                <button onClick={() => { const c = JSON.parse(JSON.stringify(cvData)); setEditData(c); setEditMode(v => !v) }}
                  className="w-full bg-yellow-600 hover:bg-yellow-700 text-white text-sm py-2.5 rounded-xl">
                  ✏️ CV'yi Düzenle
                </button>
                ...
                {editMode && editData && (
                  <div className="bg-gray-900 border border-yellow-700 rounded-xl p-4">
                    ...
                    <button onClick={handleEditSave}
                      className="w-full bg-yellow-600 hover:bg-yellow-700 text-white text-sm py-2.5 rounded-xl mt-3">✅ Kaydet</button>
                  </div>
                )}
                ...
                {showReferenceForm && (
                  <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
                    ...
                    <div className="space-y-2">
                      {[['refName','Referansın adı soyadı'],['refPosition','Referansın pozisyonu'],['refCompany','Referansın şirketi'],['relationship','İlişki (Eski müdürüm vb.)']].map(([k,p]) => (
