...
/* ─── Sabitler ─── */
const NAV_H    = 64   // px — navbar py-4 + text ≈ 64px
const NAV_CLS  = 'pt-[64px]'
const NAV_TOP  = 'top-[64px]'
...
const Ic = {
  ...
  bolt:  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>,
...
function CreateCVContent() {
  ...
  const generateCV=async(improveContent=true)=>{
    ...
    try{
      ...
      if(data.success){
        ...
        setShowPreview(false);setActiveTab('actions')
  ...
  const translate=async lang=>{
    ...
    try{
      ...
      const data=await res.json()
      if(data.success){const t={...data.translatedCV,photo:cvPhoto||undefined};setCvData(t);setPreviewData(t);setCurrentLang(lang)}else alert('Hata: '+data.error)
  ...
  const scoreCV=async()=>{
    ...
    try{const res=await fetch('/api/score-cv',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cvData})});const data=await res.json();if(data.success)setScoreData(data.scoreData);else alert('Hata: '+data.error)}catch{alert('Hata')}
    setScoring(false)
  }

  const linkedin=async()=>{
    if(!cvData) return; setGenLinkedin(true)
    try{const res=await fetch('/api/linkedin-summary',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cvData})});const data=await res.json();if(data.success)setLinkedinTxt(data.summary);else alert('Hata: '+data.error)}catch{alert('Hata')}
    setGenLinkedin(false)
  }

  const reference=async()=>{
    if(!cvData||!refInfo.refName) return; setGenRef(true)
    try{const res=await fetch('/api/reference-letter',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cvData,referenceInfo:refInfo})});const data=await res.json();if(data.success){setRefLetter(data.letter);setShowRefForm(false)}else alert('Hata: '+data.error)}catch{alert('Hata')}
    setGenRef(false)
  }
  ...
  const actionsPanel=(
    <div className="space-y-2.5">
      {!cvData
        ...
        : <>
            {/* Mobil mini önizleme — sadece mobile */}
            <div className="lg:hidden mb-4">
              <button onClick={()=>setShowPreview(v=>!v)}
                className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm transition-all ${showPreview?'bg-blue-600 border-blue-500 text-white':'bg-gray-800 border-gray-700 text-gray-300'}`}>
                {Ic.eye} {showPreview?'Önizlemeyi Kapat':'CV Önizlemesini Aç'}
              </button>
              {showPreview && (
                <div className="mt-3 rounded-xl overflow-hidden border border-gray-700 bg-white max-h-80 overflow-y-auto">
            ...
            <button onClick={downloadPDF} className="w-full flex items-center gap-3 bg-blue-600 hover:bg-blue-700 text-white text-sm py-3 px-4 rounded-xl transition-all font-medium">{Ic.dl} PDF İndir</button>
            ...
            <button onClick={shareCV} disabled={sharing}
              className="w-full flex items-center gap-3 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white text-sm py-3 px-4 rounded-xl transition-all">
              {Ic.share} {sharing?'Oluşturuluyor...':'CV\'yi Paylaş'}
            </button>
            {shareLink && (
              <div className="bg-gray-800/60 border border-green-800/50 rounded-xl p-3">
                ...
                <div className="flex gap-1.5">
                  ...
                  <button onClick={()=>{navigator.clipboard.writeText(shareLink);alert('Kopyalandı!')}} className="bg-green-600 text-white text-xs px-3 py-1.5 rounded-lg shrink-0">Kopyala</button>
                </div>
              </div>
            )}
            <button onClick={resetAll} className="w-full flex items-center gap-3 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white text-sm py-3 px-4 rounded-xl transition-all border border-gray-700/50">{Ic.spin} Yeniden Oluştur</button>
  ...
  const toolsPanel=(
    <div className="space-y-3">
      {!cvData
        ...
        : <>
            <button onClick={scoreCV} disabled={scoring} className="w-full flex items-center gap-3 bg-purple-700/80 hover:bg-purple-700 disabled:opacity-50 text-white text-sm py-3 px-4 rounded-xl transition-all">{Ic.star} {scoring?'Puanlanıyor...':'CV\'yi Puanla'}</button>
            {scoreData && (
            ...
            <div className="grid grid-cols-2 gap-1.5">
              <button onClick={()=>translate('en')} disabled={translating||currentLang==='en'}
                className="flex items-center justify-center gap-1.5 bg-gray-800 hover:bg-gray-700 disabled:opacity-40 text-gray-300 hover:text-white text-xs py-2.5 rounded-xl border border-gray-700/50">
                {Ic.globe} 🇬🇧 İngilizce
              </button>
            ...
            <button onClick={linkedin} disabled={genLinkedin} className="w-full flex items-center gap-3 bg-blue-800/60 hover:bg-blue-800 disabled:opacity-50 text-white text-sm py-3 px-4 rounded-xl transition-all">{Ic.li} {genLinkedin?'Oluşturuluyor...':'LinkedIn Özeti'}</button>
            {linkedinTxt && (
              <div className="bg-gray-800/40 border border-blue-900/40 rounded-xl p-3">
                <div className="flex justify-between items-center mb-2"><p className="text-blue-400 text-xs font-medium">LinkedIn Özeti</p><button onClick={()=>navigator.clipboard.writeText(linkedinTxt)} className="text-blue-400 text-xs border border-blue-900/50 px-2 py-0.5 rounded-md">Kopyala</button></div>
                <p className="text-gray-400 text-xs leading-relaxed line-clamp-4">{linkedinTxt}</p>
              </div>
            )}
            <button onClick={()=>setShowRefForm(v=>!v)} className="w-full flex items-center gap-3 bg-orange-700/60 hover:bg-orange-700 text-white text-sm py-3 px-4 rounded-xl transition-all">{Ic.doc} Referans Mektubu</button>
            {showRefForm && (
              <div className="bg-gray-800/40 border border-orange-800/40 rounded-xl p-3 space-y-2">
                {[['refName','Ad Soyad'],['refPosition','Pozisyon'],['refCompany','Şirket'],['relationship','İlişki']].map(([k,p])=>(
                  <input key={k} type="text" placeholder={p} value={refInfo[k]} onChange={e=>setRefInfo({...refInfo,[k]:e.target.value})}
                ...
                <button onClick={reference} disabled={genRef||!refInfo.refName} className="w-full bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white text-xs py-2 rounded-lg">{genRef?'Oluşturuluyor...':'Oluştur'}</button>
              </div>
            )}
            {refLetter && (
              <div className="bg-gray-800/40 border border-orange-900/40 rounded-xl p-3">
                <div className="flex justify-between items-center mb-2"><p className="text-orange-400 text-xs font-medium">Referans Mektubu</p><button onClick={()=>navigator.clipboard.writeText(refLetter)} className="text-orange-400 text-xs border border-orange-900/50 px-2 py-0.5 rounded-md">Kopyala</button></div>
                <p className="text-gray-400 text-xs leading-relaxed whitespace-pre-line line-clamp-5">{refLetter}</p>
              </div>
            )}
          </>
      }
  ...
  return (
    /*
      TEMEL DÜZELTME: Sayfanın kendisi overflow gizli, sağ panel scroll ediyor.
      Navbar fixed olduğu için pt-[64px] ile offset veriyoruz.
      Sticky panel top-[64px] ile navbar'ın tam altından başlıyor.
    */
    <div className={`bg-gray-950 ${NAV_CLS}`}>
      <input ref={fileRef} type="file"
        accept=".pdf,.doc,.docx,.txt,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain"
        className="hidden" onChange={handleFileUpload}/>

      {/* ── BAŞLANGIÇ: seçim ekranı ─────────────────────────────────── */}
      {startMode===null && (
        /*
          fixed inset-0 yerine min-h-screen + overflow-y-auto kullanıyoruz.
          Böylece: navbar arkasında kalmaz, scroll çalışır, mobilde taşmaz.
        */
        <div className="min-h-[calc(100vh-64px)] flex flex-col items-center justify-center px-4 py-8 overflow-y-auto">
          <div className="w-full max-w-md">
            ...
            <div className="space-y-3 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-3">
              <button onClick={()=>{setStartMode('upload');setTimeout(()=>fileRef.current?.click(),50)}}
                ...
                <div className="w-12 h-12 bg-blue-600/15 border border-blue-600/20 rounded-xl flex items-center justify-center mb-4 group-hover:bg-blue-600/25 transition-all">
                  <svg viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth={1.5} className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </div>
                <h3 className="text-white font-bold text-base mb-1.5">Belge Yükle</h3>
                <p className="text-gray-500 text-xs leading-relaxed">Mevcut CV'nizi yükleyin, AI profesyonel hale getirir.</p>
              ...
              <button onClick={()=>setStartMode('text')}
                ...
                <div className="w-12 h-12 bg-purple-600/15 border border-purple-600/20 rounded-xl flex items-center justify-center mb-4 group-hover:bg-purple-600/25 transition-all">
                  <svg viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth={1.5} className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                </div>
                <h3 className="text-white font-bold text-base mb-1.5">Manuel Yaz</h3>
                <p className="text-gray-500 text-xs leading-relaxed">Kısa notlar yazın, AI profesyonel CV'ye dönüştürsün.</p>
                <div className="mt-3"><span className="text-xs bg-purple-900/30 text-purple-400 px-2.5 py-0.5 rounded border border-purple-800/40">AI ile tamamlar</span></div>
              </button>
            </div>
      ...
      {startMode==='text' && !loading && !cvData && (
        <div className="min-h-[calc(100vh-64px)] flex flex-col justify-center px-4 py-8">
          <div className="w-full max-w-lg mx-auto">
            <button onClick={()=>setStartMode(null)} className="flex items-center gap-1.5 text-gray-600 hover:text-white text-sm mb-6 transition-all">{Ic.back} Geri</button>
            ...
            <button onClick={()=>generateCV(true)} disabled={!cvText.trim()}
              className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white font-semibold py-3 rounded-2xl text-sm">
              ✨ CV Oluştur
            </button>
          </div>
        </div>
      ...
      {showImprove && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm px-4 pb-6 sm:pb-0">
          <div className="bg-gray-900 border border-gray-700/60 rounded-2xl p-5 w-full max-w-sm shadow-2xl">
            ...
            <p className="text-gray-500 text-sm text-center mb-4 leading-relaxed">Yapay zeka içeriği profesyonel hale getirsin mi?</p>
            ...
            {pendingHasPhoto && !pendingPhoto64 && <div className="bg-blue-900/20 border border-blue-800/40 rounded-xl px-3 py-2 mb-3"><p className="text-blue-400 text-xs">📸 PDF'de fotoğraf var, Fotoğraf sekmesinden yükleyebilirsiniz.</p></div>}
      ...
      {(startMode==='done'||isEdit) && (
        /*
          SCROLL DÜZELTMESİ:
          - Dış wrapper: overflow-hidden, tam viewport yüksekliği
          - Sol: sticky, sabit yükseklikte, kendi içinde scroll
          - Sağ: flex-1, kendi içinde overflow-y-auto (sadece sağ scroll eder)
          - Böylece sol panel asla navbar'ın arkasına gitmiyor
        */
        <div className={`flex ${NAV_H_CLS} overflow-hidden`}>

          {/* Dikey icon bar — desktop only */}
          <div className="hidden lg:flex flex-col items-center gap-1 py-4 px-2 bg-[#0d1117] border-r border-gray-800/80 w-14 shrink-0">
            <button onClick={()=>router.push('/dashboard')} title="Geri"
              className="w-9 h-9 flex items-center justify-center rounded-xl text-gray-600 hover:text-white hover:bg-gray-800 transition-all mb-3">
              {Ic.back}
            </button>
          ...
          {/* Panel — sol, sabit, kendi içinde scroll */}
          <div className="w-full lg:w-72 xl:w-80 shrink-0 bg-gray-950 border-r border-gray-800/60 flex flex-col overflow-hidden">

            {/* Mobil header */}
            <div className="lg:hidden flex-shrink-0 bg-gray-950 border-b border-gray-800/60">
              <div className="flex items-center gap-2 px-3 py-2.5">
                <button onClick={()=>router.push('/dashboard')}
                  className="flex items-center gap-1.5 text-gray-500 hover:text-white text-xs bg-gray-800 px-2.5 py-1.5 rounded-lg border border-gray-700/40">
                  {Ic.back} Geri
                </button>
              ...
              {/* Tab bar */}
              <div className="flex gap-0.5 px-3 pb-2 overflow-x-auto" style={{scrollbarWidth:'none'}}>
                {TABS.map(tab=>(
                  <button key={tab.id} onClick={()=>setActiveTab(tab.id)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap shrink-0 transition-all ${activeTab===tab.id?'bg-blue-600 text-white':'text-gray-500 hover:text-white hover:bg-gray-800/60'}`}>
                    <span className="w-3.5 h-3.5 shrink-0">{tab.icon}</span>{tab.label}
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Desktop başlık */}
            <div className="hidden lg:flex items-center flex-shrink-0 px-4 py-3 border-b border-gray-800/40">
          ...
          {/* Sağ: Önizleme — sadece sağ taraf scroll eder */}
